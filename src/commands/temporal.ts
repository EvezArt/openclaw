...
import { info, theme } from "../terminal/theme.js";
 ...
 */
function runTemporalSimulation(params: Required<TemporalExperimentParams>): TemporalExperimentResult {
  ...
  for (let i = 0; i < params.iterations; i++) {
    // Compute current satisfaction of past constraint (distance from past target)
    const pastError = Math.abs(state - pastTarget);
    
    // Compute current satisfaction of future constraint (distance from future target)
    const futureError = Math.abs(state - futureTarget);
...
export async function temporalCommand(
  ...
  if (config.json) {
    ...
    runtime.log("");
    runtime.log(theme.cyan("═══════════════════════════════════════════"));
    runtime.log(theme.cyan("  Temporal Experiment Simulation"));
    runtime.log(theme.cyan("  (Classical Simulation Only)"));
    runtime.log(theme.cyan("═══════════════════════════════════════════"));
    runtime.log("");
